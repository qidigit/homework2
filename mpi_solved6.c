...
*
*   rank 2 use MPI_Send so it is improper to use MPI_Waitall for this process.
*   also offset for rank 1 should not start with REPS, leading to error in memory 
*   allocation.
* AUTHOER: Blaise Barney, corrected by Di Qi 
...
if (rank < 2) {
  ...
  if (rank == 1) {
    src = 0;
    // the previous initialization of offset here is wrong!
    offset = 0; //REPS;
    }
  dest = src;
...
//MPI_Wait for process rank 2 is improper.
